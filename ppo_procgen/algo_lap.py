...
def combined_loss(
    ...
    out = jnp.array_split(apply_fn(params_model, obs_stack, eigenvals,  False)[-1], 3)
    ...
    model_out, bn_updates =  apply_fn({'params':params_model['params'], 'batch_stats':batch_stats}, state, eigenvals, True, mutable=['batch_stats'])

    value_pred, pi, _ = model_out
    ...
    return ppo_items[0] + 1e-4*lap_items[0], (lap_items, (ppo_items, bn_updates))
...
@jax.jit
def policy_f(train_state: TrainState,
           state: np.ndarray):
    model_out, bn_up = train_state.apply_fn({'params':train_state.params['params'], 'batch_stats':train_state.batch_stats}, state, -1*jnp.diag(train_state.betas), True, mutable=['batch_stats'])
    train_state.replace(batch_stats=bn_up['batch_stats'])
    value, pi, _ = model_out
    return value, pi, train_state
...
def policy(apply_fn: Callable[..., Any],
           ...
           state: np.ndarray,
           eigenvals):
    value, pi, _ = apply_fn(params, state, eigenvals, False)
    return value, pi
...
def select_action_reg(
    ...
    if train:
        value, pi, train_state = policy_f(train_state, state)
    else:
        value, pi = policy(train_state.apply_fn, train_state.params, state, -1*jnp.diag(train_state.betas))
    ...
    if not train:
        ...
        return action, log_prob, value[:, 0], rng, train_state
...
def get_transition_reg(
    ...
    if not train:
        action, log_pi, value, new_key = select_action_reg(
            train_state, state.astype(jnp.float32) / 255., rng, sample=True, train=train)
    else:
        action, log_pi, value, new_key, train_state = select_action_reg(
            train_state, state.astype(jnp.float32) / 255., rng, sample=True, train=train)
...
def update_regularizer(
    ...
    for _ in range(epoch_ppo):
        ...
        total_loss, value_loss, loss_actor, entropy, value_pred, target_val, gae_val = items[1][1][0]
    ...
    for idx in idxes:
        ...
        items, grads = grad_fn(train_state.params, train_state.apply_fn, state[idx], next_state[idx], neg_obs,
                                   target[idx],
                                    ...
                                    critic_coeff=critic_coeff,
                                    entropy_coeff=entropy_coeff,
                                    eigenvals=-1*jnp.diag(train_state.betas),
                                    batch_stats=train_state.batch_stats)
        
        train_state.replace(batch_stats=items[1][1][1]['batch_stats'])
        train_state = train_state.apply_gradients(grads=grads)
