...
class Batch:
    ...
    def __init__(
        ...
        self.lap_embed_size = lap_embed_size

        self.reset()

    def reset(self):
        #here states mean the laplacian embeddings
        self.states = np.empty((self.n_steps, self.num_envs, self.lap_embed_size))

        self.actions = np.empty((self.n_steps, self.num_envs), dtype=np.float32)
        self.rewards = np.empty((self.n_steps, self.num_envs), dtype=np.float32)
        self.dones = np.empty((self.n_steps, self.num_envs), dtype=np.float32)
        ...
        self.obs = np.empty((self.n_steps, self.num_envs, *self.state_shape), dtype=np.float32)
        self.next_obs = np.empty((self.n_steps, self.num_envs, *self.state_shape), dtype=np.float32)

    def append(self, lap_embed, action, reward, done, log_pi, value, obs, next_obs):
        self.states[self._p] = lap_embed
        ...
        self.obs[self._p] = obs
        self.next_obs[self._p] = next_obs
...
class BatchReg:
    ...
    def reset(self):
        ...
        self.next_states = np.empty((self.n_steps, self.num_envs, *self.state_shape), dtype=np.uint8)
    ...
    def append(self, state, next_state, action, reward, done, log_pi, value):
