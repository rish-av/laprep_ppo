...
from algo_lap_reg import update_regularizer, get_transition_reg, select_action_reg
...
flags.DEFINE_float("lap_lr", 5e-4, "laplacian learning rate")
...
flags.DEFINE_float("entropy_coeff", 0.005, "Entropy loss coefficient")
...
flags.DEFINE_string("method_name","feature_fusion", "Name of the method")
...
def main():
        ...
        print('Training for seed {}'.format(FLAGS.seed))
        ...
        wandb.init(project='lap_reg_ppo', entity='revisit-spectral-rl', config=FLAGS, name='feature_fusion_ppo_{}_{}'.format(FLAGS.env_name, FLAGS.seed), 
        ...
        model = FeatureFusion(embedding_dim=lap_embed_dim, action_dim=n_actions, inverse=False, diff_power=0)
        params_model = model.init(key, jnp.zeros((1, 64, 64, 3)), train=False)
        ...
        model_dir = os.path.join(FLAGS.model_dir, FLAGS.env_name + '_' + FLAGS.method_name + str(FLAGS.seed))

        for step in tqdm.tqdm(range(1, int(FLAGS.train_steps // FLAGS.num_envs + 1))):

            #ppo training
            train_state, state, batch, key = get_transition_reg(train_state, env, state, batch, key)

            action_id, _, _, key = select_action_reg(train_state, state_id.astype(jnp.float32) / 255., key, sample=True)
            state_id, _, _, infos_id = env_test_ID.step(action_id)

            action_ood, _, _, key = select_action_reg(train_state, state_ood.astype(jnp.float32) / 255., key, sample=True)
